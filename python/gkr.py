...
class Proof:
    ...
    def to_dict(self):
        to_serialize = dict()
        to_serialize['sumcheckProof'] = list(
            map(lambda x: list(map(lambda y: list(map(lambda z: repr(z), y)), x)), self.sumcheck_proofs))
        ...
        to_serialize['add'] = list(
            map(lambda x: list(map(lambda y: list(map(lambda z: repr(z), y)), x)), self.add))
        to_serialize['mult'] = list(
            map(lambda x: list(map(lambda y: list(map(lambda z: repr(z), y)), x)), self.mult))
...
def prove(circuit: Circuit, D, epsilon: field.FQ = field.FQ.zero()):
    ...
    for i in range(circuit.depth() - 1):
        ...
        w_i_ext_c = get_ext_from_k(circuit.w_i(i + 1), circuit.k_i(i + 1),
                                   circuit.k_i(i) + circuit.k_i(i + 1) + 1)
...
def prove_approx(circuit: Circuit, D, max_delta: field.FQ = field.FQ.zero()):
    ...
    Args:
        ...
        D: Output values
        max_delta: Base error budget (δ) used to derive per-round deltas
    ...
    for i in range(circuit.depth() - 1):
        ...
        # Bind r_star to the verifier-fixed base delta
...
def verify_approx(proof: ApproxProof, max_delta: field.FQ, total_epsilon: Optional[field.FQ] = None):
    """
    Approximate GKR verifier for ApproxProof.

    Verifier-fixed error budget: `max_delta` is provided by the verifier,
    and per-round deltas are derived via `round_delta(max_delta, round_idx)`.
    """
    start = time.time()
    m = [field.FQ.zero()] * proof.d
    m[0] = eval_expansion(proof.D, proof.z[0])
    ...
    if total_epsilon is None:
        total_epsilon = field.FQ.zero()
        for layer_idx in range(proof.d - 1):
            v = 2 * proof.k[layer_idx + 1]
            total_epsilon = total_epsilon + sum([round_delta(max_delta, j) for j in range(v)], field.FQ.zero())
    ...
    for i in range(proof.d - 1):
        if proof.deltas and len(proof.deltas) > i:
            ...
            valid, layer_error = verify_approx_sumcheck(
                m[i], approx_sumcheck_proof, 2 * proof.k[i + 1], base_delta=max_delta
            )
        ...
        f_error = abs_field(proof.f[i] - modified_f)
        if int(f_error) > int(total_epsilon) or sumcheck_p_hash != proof.r[i]:
            print("verifying time :", time.time() - start)
            return False, total_error
    ...
    if int(total_error) > int(total_epsilon):
        print("verifying time :", time.time() - start)
        return False, total_error
...
def verify_approx(proof: ApproxProof, max_delta: field.FQ, total_epsilon: Optional[field.FQ] = None):
    """
    Approximate GKR verifier for ApproxProof.
    Verifier-fixed error budget (paper-aligned): `max_delta` is provided by the verifier,
    and per-round deltas are derived via `round_delta(max_delta, round_idx)`.
    ...
    # Derive (verifier-fixed) total epsilon unless explicitly provided.
    if total_epsilon is None:
        total_epsilon = field.FQ.zero()
        for layer_idx in range(proof.d - 1):
            v = 2 * proof.k[layer_idx + 1]
    ...
    for i in range(proof.d - 1):
        ...
        if proof.deltas and len(proof.deltas) > i:
            hash_input = list(map(lambda x: int(x), sumcheck_p[-1]))
            # Bind r_star to the verifier-fixed base delta (not a prover-chosen per-round delta).
            hash_input.append(int(max_delta))
            sumcheck_p_hash = field.FQ(mimc.mimc_hash(hash_input))
        else:
            sumcheck_p_hash = field.FQ(mimc.mimc_hash(list(map(lambda x: int(x), sumcheck_p[-1]))))
